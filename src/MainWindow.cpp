	...
	: QMainWindow(parent), m_Volume(0)
{
	...
	connect(m_Ui->box_Iso, SIGNAL(valueChanged(double)), this, SLOT(isoValueChanged(double)));
...
{
	m_glWidget->m_camera.setFOV(value);
	m_glWidget->update();
}
...
void MainWindow::isoValueChanged(double value)
{
	m_glWidget->m_iso = value;
	m_glWidget->update();
}

void MainWindow::nearPlaneChanged(double value)
{
	m_glWidget->m_camera.setNear(value);
	m_glWidget->update();
}

void MainWindow::farPlaneChanged(double value)
{
	m_glWidget->m_camera.setFar(value);
	m_glWidget->update();
}
